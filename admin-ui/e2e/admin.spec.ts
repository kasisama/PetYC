...
async function mockAdminApi(page: Page) {
  ...
  await page.route('**/api/admin/**', async (route) => {
    ...
    if(path==='/api/admin/content/messages') return ok([{key:'pet.status',description:'宠物状态',template:'🐾 {pet_name}的近况',variables:{pet_name:'光芽兽'},sample:'🐾 光芽兽的近况'}])
    if(path.startsWith('/api/admin/content/messages/')&&path.endsWith('/preview')) return ok({key:'pet.status',platform:'onebot',sender:'QQ 宠物机器人',text:'🐾 光芽兽的近况'})
    ...
    if(path==='/api/admin/config/menus'&&request.method()==='PUT') return ok(null)
    if(path==='/api/admin/config/menus') return ok([{Name:'主菜单',Reply:'【宠物远征生态】\n状态｜今日｜远征',Image:'上传/main.webp'}])
...
test('成长零数据与内容配置均为完整产品页面',async({page},testInfo)=>{
  ...
  await expect(page.getByRole('heading',{name:'活动名称与运营状态'})).toBeVisible()
  await expect(page.getByRole('heading',{name:'故事选项与实际效果'})).toBeVisible()
  await expect(page.getByRole('heading',{name:'累计进度达到多少时发什么'})).toBeVisible()
...
test('菜单场景可预览并上传配图',async({page})=>{
  ...
  const card=page.locator('.menu-card').filter({hasText:'主菜单'})
  ...
  const saveRequest=page.waitForRequest((request)=>request.url().includes('/api/admin/config/menus')&&request.method()==='PUT')
  await page.getByRole('button',{name:'保存当前配置'}).click()
  const payload=await (await saveRequest).postDataJSON()
  expect(payload[0].Image).toBe('上传/main-menu.webp')
})

test('菜单场景可清除配图并保存',async({page})=>{
  await page.goto('content?tab=text')
  await page.getByRole('button',{name:'菜单场景'}).click()
  const card=page.locator('.menu-card').filter({hasText:'主菜单'})
  await card.getByRole('button',{name:'编辑'}).click()
  ...
  await page.getByRole('button',{name:'保存当前配置'}).click()
  const payload=await (await saveRequest).postDataJSON()
  expect(payload[0].Image).toBe('')
})

test('赛季重置危险操作需要确认文案',async({page})=>{
  await page.goto('system')
  await expect(page.getByRole('heading',{name:'系统设置',exact:true})).toBeVisible()
  await page.getByRole('button',{name:'重置赛季进度'}).click()
  const dialog=page.getByRole('dialog',{name:'确认重置赛季'})
  await expect(dialog).toBeVisible()
  await dialog.getByRole('button',{name:'确认重置赛季'}).click()
  await expect(dialog.getByText('请填写操作原因')).toBeVisible()
  await dialog.locator('textarea').fill('首季结算验收')
  await dialog.getByRole('button',{name:'确认重置赛季'}).click()
  await expect(dialog.getByRole('alert')).toContainText('请输入「重置赛季:')
})
