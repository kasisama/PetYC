...
import { computed, onErrorCaptured, onMounted, ref, toRaw } from 'vue'
...
type FieldType = 'text'|'textarea'|'number'|'toggle'|'select'|'datetime'|'image'
...
const fields:Record<CollectionKey, CollectionDefinition> = {
  maps:{key:'maps',label:'大地图',singular:'大地图',description:'配置世界入口、地图主题、推荐等级和开放状态。',titleField:'name',keyField:'key',defaults:{name:'',region:'',description:'',image:'',recommended_level:1,enabled:true,sort_order:0},fields:[
    {key:'name',label:'地图名称',type:'text',required:true},{key:'region',label:'世界区域',type:'text',required:true},{key:'description',label:'玩家说明',type:'textarea',wide:true},{key:'image',label:'地图封面',type:'image',help:'直接上传地图封面；可随时移除或重新上传',wide:true},{key:'recommended_level',label:'推荐挑战等级',type:'number',min:1},yesNo(),{key:'sort_order',label:'显示顺序',type:'number'}]},
  zones:{key:'zones',label:'区域',singular:'区域',description:'配置地图内的可探索区域、消耗、难度和远征解锁目标。',titleField:'name',keyField:'key',defaults:{map_key:'',name:'',description:'',image:'',recommended_level:1,difficulty_permille:1000,hunger_cost:0,readiness_cost:0,expedition_unlock_objective_key:'',exploration_mode:'legacy',enabled:true,sort_order:0},fields:[
    {key:'map_key',label:'所属大地图',type:'select',source:'maps',required:true},{key:'name',label:'区域名称',type:'text',required:true},{key:'description',label:'区域说明',type:'textarea',wide:true},{key:'image',label:'区域图片',type:'image',wide:true},{key:'recommended_level',label:'推荐等级',type:'number',min:1},{key:'difficulty_permille',label:'难度系数',type:'number',min:1,help:'1000 表示标准难度，1200 表示 1.2 倍'},{key:'hunger_cost',label:'单次探索饥饿消耗',type:'number',min:0},{key:'readiness_cost',label:'单次探索体力消耗',type:'number',min:0},{key:'exploration_mode',label:'探索模式',type:'select',options:choice([['legacy','传统目标'],['node','节点事件']]),help:'节点事件模式必须配置阶段、事件和选项'},{key:'expedition_unlock_objective_key',label:'远征解锁目标',type:'select',source:'objectives',help:'传统目标模式使用；节点事件模式在区域收束后解锁'},yesNo(),{key:'sort_order',label:'显示顺序',type:'number'}]},
  ...
  stages:{key:'stages',label:'探索阶段',singular:'探索阶段',description:'节点模式区域的阶段、线索门槛、进度区间和必经事件。',titleField:'name',keyField:'key',defaults:{zone_key:'',name:'',description:'',progress_start:0,progress_end:25,required_clues:0,next_stage_key:'',event_key:'',enabled:true,sort_order:0},fields:[
    {key:'zone_key',label:'所属区域',type:'select',source:'zones',required:true},{key:'name',label:'阶段名称',type:'text',required:true},{key:'description',label:'玩家目标说明',type:'textarea',wide:true},{key:'progress_start',label:'起始进度',type:'number',min:0,max:100},{key:'progress_end',label:'结束进度',type:'number',min:1,max:100},{key:'required_clues',label:'所需线索',type:'number',min:0},{key:'next_stage_key',label:'完成后的下一阶段',type:'select',source:'stages'},{key:'event_key',label:'进入时事件',type:'select',source:'story_events'},yesNo(),{key:'sort_order',label:'顺序',type:'number'}]},
  story_events:{key:'story_events',label:'探索事件',singular:'探索事件',description:'配置不期而遇、命运所指与区域收束的事件文案。',titleField:'name',keyField:'key',defaults:{zone_key:'',stage_key:'',name:'',description:'',event_type:'mainline',weight:1,enabled:true,sort_order:0},fields:[
    {key:'zone_key',label:'所属区域',type:'select',source:'zones',required:true},{key:'stage_key',label:'触发阶段',type:'select',source:'stages'},{key:'name',label:'事件标题',type:'text',required:true},{key:'description',label:'事件正文',type:'textarea',wide:true},{key:'event_type',label:'事件类型',type:'select',options:choice([['mainline','主线事件'],['repeat','通关后邂逅']])},{key:'weight',label:'出现权重',type:'number',min:1},yesNo(),{key:'sort_order',label:'顺序',type:'number'}]},
  story_choices:{key:'story_choices',label:'事件选项',singular:'事件选项',description:'每个主线事件配置 2–3 个选择；选择推进到对应的下一阶段。',titleField:'label',defaults:{event_key:'',choice_key:'',label:'',description:'',risk_level:'low',next_stage_key:'',enabled:true,sort_order:0},fields:[
    {key:'event_key',label:'所属事件',type:'select',source:'story_events',required:true},{key:'label',label:'按钮文字',type:'text',required:true},{key:'description',label:'结果说明',type:'textarea',wide:true},{key:'risk_level',label:'风险级别',type:'select',options:choice([['low','低风险'],['medium','有风险'],['high','高风险']])},{key:'next_stage_key',label:'选择后的阶段',type:'select',source:'stages',help:'留空表示完成区域'},yesNo(),{key:'sort_order',label:'顺序',type:'number'}]},
  ...
  monsters:{key:'monsters',label:'怪物',singular:'怪物',description:'配置等级、战斗属性、AI 与战后奖励。',titleField:'name',keyField:'key',defaults:{name:'',description:'',image:'',level:1,max_health:100,attack:10,defense:5,wisdom:0,adventure_xp:10,ai_profile:'balanced',fixed_loot_pool_key:'',random_loot_pool_key:'',elite:false,enabled:true},fields:[
    {key:'name',label:'怪物名称',type:'text',required:true},{key:'description',label:'怪物说明',type:'textarea',wide:true},{key:'image',label:'怪物图片',type:'image',wide:true},{key:'level',label:'等级',type:'number',min:1},{key:'max_health',label:'生命',type:'number',min:1},{key:'attack',label:'攻击',type:'number',min:0},{key:'defense',label:'防御',type:'number',min:0},{key:'wisdom',label:'灵性',type:'number',min:0},{key:'adventure_xp',label:'冒险经验',type:'number',min:0},{key:'ai_profile',label:'战斗 AI',type:'select',options:choice([['balanced','均衡'],['aggressive','进攻'],['defensive','防守'],['tactical','策略']])},{key:'fixed_loot_pool_key',label:'固定奖励池',type:'select',source:'loot_pools'},{key:'random_loot_pool_key',label:'随机奖励池',type:'select',source:'loot_pools'},{key:'elite',label:'精英怪物',type:'toggle'},yesNo()]},
  ...
  shop_items:{key:'shop_items',label:'远征商品',singular:'远征商品',description:'固定商品使用调查徽章结算，并按玩家独立计算限购。',titleField:'name',keyField:'key',defaults:{name:'',description:'',image:'',product_type:'item',product_key:'',quantity:1,price:1,limit_type:'none',limit_quantity:0,enabled:true,sort_order:0},fields:[
    {key:'name',label:'商品名称',type:'text',required:true},{key:'description',label:'玩家说明',type:'textarea',wide:true},{key:'image',label:'商品图片',type:'image',wide:true},{key:'product_type',label:'出售类型',type:'select',options:choice([['item','统一物品'],['equipment','装备成品'],['blueprint_fragment','蓝图碎片']])},{key:'product_key',label:'出售对象',type:'select',dynamic:'product-target',required:true},{key:'quantity',label:'单次发放数量',type:'number',min:1},{key:'price',label:'调查徽章价格',type:'number',min:0},{key:'limit_type',label:'限购周期',type:'select',options:choice([['none','不限购'],['daily','每日'],['weekly','每周'],['lifetime','永久']])},{key:'limit_quantity',label:'周期限购数量',type:'number',min:0,hiddenWhen:r=>getField(r,'limit_type')==='none'},yesNo(),{key:'sort_order',label:'显示顺序',type:'number'}]},
  ...
  equipment_templates:{key:'equipment_templates',label:'装备',singular:'装备',description:'配置武器、防具与秘宝的基础属性、词条和分解产物。',titleField:'name',keyField:'key',defaults:{name:'',description:'',image:'',slot:'weapon',rarity:'common',required_level:1,base_attack:0,base_defense:0,base_health:0,base_wisdom:0,affix_pool_key:'',min_affixes:0,max_affixes:0,salvage_item:'',salvage_quantity:0,enabled:true},fields:[
    {key:'name',label:'装备名称',type:'text',required:true},{key:'slot',label:'装备槽位',type:'select',options:choice([['weapon','武器'],['armor','防具'],['treasure','秘宝']])},{key:'rarity',label:'稀有度',type:'select',options:choice([['common','普通'],['fine','优秀'],['rare','稀有'],['epic','史诗'],['legendary','传说']])},{key:'description',label:'装备说明',type:'textarea',wide:true},{key:'image',label:'装备图片',type:'image',wide:true},{key:'required_level',label:'穿戴等级',type:'number',min:1},{key:'base_attack',label:'基础攻击',type:'number',min:0},{key:'base_defense',label:'基础防御',type:'number',min:0},{key:'base_health',label:'基础生命',type:'number',min:0},{key:'base_wisdom',label:'基础灵性',type:'number',min:0},{key:'affix_pool_key',label:'词条池',type:'select',source:'equipment_affixes'},{key:'min_affixes',label:'最少词条',type:'number',min:0},{key:'max_affixes',label:'最多词条',type:'number',min:0},{key:'salvage_item',label:'分解产物',type:'select',source:'items'},{key:'salvage_quantity',label:'分解数量',type:'number',min:0},yesNo()]},
...
const modules:ModuleDefinition[] = [
  ...
  {key:'exploration',label:'探索机制',description:'传统目标与节点事件共存；节点区域可配置阶段、剧情和风险分支。',collections:['objectives','stages','story_events','story_choices','encounters','encounter_effects']},
...
const loading=ref(true), saving=ref(false), uploading=ref(false), error=ref(''), editorError=ref(''), search=ref('')
...
function sourceOptions(source:CollectionKey):Option[]{return optionRows(source).map(row=>({value:String(getField(row,fields[source].keyField ?? fields[source].titleField) ?? ''),label:rowTitleFor(source,row),group:sourceOptionContext(source,row)}))}
...
function sourceOptionContext(source:CollectionKey,row:AdventureEntity){const scope=String(getField(row,source==='zones'?'map_key':source==='objectives'||source==='stages'||source==='story_events'?'zone_key':'')||'');const disabled=getField(row,'enabled')===false?'已停用':'';return [scope,disabled].filter(Boolean).join(' · ') || undefined}
...
function createRow(){const def=definition.value;const value={...structuredClone(def.defaults)};if(def.keyField)Reflect.set(value,def.keyField,safeKey(def.key.replaceAll('_','-')));if(def.key==='story_choices')Reflect.set(value,'choice_key',safeKey('story-choice'));draft.value=value as AdventureEntity;editingIndex.value=-1;editorError.value='';drawerOpen.value=true}
function editRow(row:AdventureEntity){const index=currentRows.value.indexOf(row);if(index<0){toast.error('未能定位要编辑的条目，请刷新后重试');return}editingIndex.value=index;draft.value=structuredClone(toRaw(row));editorError.value='';drawerOpen.value=true}
...
async function load(){loading.value=true;error.value='';try{const data=await getAdventureCatalog();revision.value=data.revision;catalog.value=data.catalog;snapshot.value=JSON.stringify(data.catalog);issues.value=[]}catch(reason){error.value=reason instanceof ApiError?reason.message:'冒险配置读取失败'}finally{loading.value=false}}
...
async function uploadDraftImage(file:File,field:string){if(!draft.value)return;const supported=new Set(['image/png','image/jpeg','image/gif','image/webp']);if(!supported.has(file.type)){toast.error('只支持 JPG、PNG、GIF 或 WEBP 图片');return}if(file.size>10*1024*1024){toast.error('图片不能超过 10MB');return}uploading.value=true;try{const result=await uploadImage(file);setField(draft.value,field,result.path);toast.success('图片已上传，保存草稿后生效')}catch(reason){toast.error(reason instanceof Error?reason.message:'图片上传失败')}finally{uploading.value=false}}
function clearDraftImage(field:string){if(draft.value)setField(draft.value,field,'')}
function retryEditor(){editorError.value=''}
onErrorCaptured((reason)=>{if(!drawerOpen.value)return;const errorCode=`ADV-${activeCollection.value.toUpperCase()}-${Date.now().toString(36).toUpperCase()}`;console.error('冒险编辑器渲染失败',{errorCode,page:'adventure',collection:activeCollection.value,editingIndex:editingIndex.value,revision:revision.value},reason);editorError.value=errorCode;return false})
onMounted(load)
</script>
...
<template>
  <section class="adventure-page">
    ...
    <UiDrawer :open="drawerOpen" :title="editingIndex<0?`新增${definition.singular}`:`编辑${rowTitle(draft!)}`" :description="definition.description" @close="drawerOpen=false">
      <div v-if="editorError" class="editor-fallback" role="alert"><b>编辑器加载失败</b><p>当前条目没有被修改。请重试；若问题持续，请将错误编号提供给支持人员。</p><code>错误编号：{{ editorError }}</code><div><button class="btn btn-ghost" type="button" @click="drawerOpen=false">关闭编辑器</button><button class="btn btn-primary" type="button" @click="retryEditor">重试</button></div></div>
      <form v-else-if="draft" class="entity-form" @submit.prevent="saveRow">
        <template v-for="field in definition.fields" :key="field.key">
          <label v-if="!field.hiddenWhen?.(draft)" :class="{wide:field.wide,toggle:field.type==='toggle'}">
            ...
            <template v-else><span>{{field.label}}<i v-if="field.required">必填</i></span>
              ...
              <ImageDropzone v-else-if="field.type==='image'" :path="String(getField(draft,field.key)??'')" :label="`${definition.singular}${field.label}`" kind="image" size="medium" :busy="uploading" @file="uploadDraftImage($event,field.key)" @clear="clearDraftImage(field.key)"/>
              <UiSearchSelect v-else-if="field.type==='select'" :model-value="String(getField(draft,field.key)??'')" :options="fieldOptions(field,draft)" placeholder="请选择或输入名称搜索" @update:model-value="setField(draft,field.key,$event)"/>
        ...
        <details v-if="editingIndex>=0 && definition.keyField" class="technical-field wide"><summary>高级设置</summary><label><span>内部稳定标识（只读）</span><input :value="String(getField(draft,definition.keyField)??'')" readonly /></label></details>
        <details v-if="editingIndex>=0 && activeCollection==='story_choices'" class="technical-field wide"><summary>高级设置</summary><label><span>内部选项标识（只读）</span><input :value="String(getField(draft,'choice_key')??'')" readonly /></label></details>
      </form>
...
.entity-list{grid-template-columns:repeat(2,minmax(0,1fr))}
</style>
