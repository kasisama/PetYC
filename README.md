...
当前已发布版本：`v0.1.1` · `v0.1.2` 开发中，完整内容见 [更新日志](CHANGELOG.md)。
...
```powershell
cd D:\PetYC
.\petyc_0.1.2_windows_amd64.exe
```

按 `Ctrl+C` 可以让服务正常关闭。查看版本：

```powershell
.\petyc_0.1.2_windows_amd64.exe --version
```
...
sudo mv petyc_0.1.2_linux_amd64 /opt/petyc/
sudo chmod +x /opt/petyc/petyc_0.1.2_linux_amd64
cd /opt/petyc
./petyc_0.1.2_linux_amd64
```
...
ExecStart=/opt/petyc/petyc_0.1.2_linux_amd64
Restart=on-failure
RestartSec=5
TimeoutStopSec=30
