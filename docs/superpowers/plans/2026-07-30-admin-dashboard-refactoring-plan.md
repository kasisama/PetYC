...
| 3. 初始化 Vue 3 前端工程 | ✅ 已完成 | `eb1e810` |
| 4. 主题控制器与全局 Layout | ✅ 已完成 | `51a0933` |
| 5. Vue 登录与验证闭环 | ✅ 已完成 | `51a0933` |

**当前状态：** 计划内的 5 个任务全部完成。`/admin` 已切换为 Vue 3 SPA，构建产物
（98.40 kB JS + 5.69 kB CSS）经 `go:embed` 打进二进制，替换了原先 188KB 的单文件面板。
登录、会话校验、改密、退出、主题切换与仪表盘统计均已闭环；`ConfigView` / `PetsView` /
`AssetsView` 目前是占位页，等待后续任务补全表单与列表。
